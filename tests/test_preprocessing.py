from src.data.preprocess import pre_process_zinc, canonicalize_and_drop, add_atom_counts, add_bond_counts, add_ring_counts, add_property 
from src.utils.config import get_data_info
from src.utils.mol_utils import mols_from_smiles
...
def test_add_molecular_properties():
    input_json_str = """
        [
            {
                "smiles":"CC(C)(C)c1ccc2occ(CC(=O)Nc3ccccc3F)c2c1",
    ...
    dataset_expected = pd.read_json(StringIO(expected_output_json_str), orient = "records")
    pd.testing.assert_frame_equal(dataset, dataset_expected)
